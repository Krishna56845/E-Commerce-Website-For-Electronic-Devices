...
function CheckoutForm({ onSubmit, submitting = false }) {
  ...
  const handleInputChange = e => {
    ...
    switch (name) {
      ...
      case 'cvv': {
        ...
        const cardTypeForcvv = getCardType(cardDigits);
        const requiredcvvLength = cardTypeForcvv === 'American Express' ? 4 : 3;

        if (sanitizedValue.length > 0 && sanitizedValue.length < requiredcvvLength) {
          newValidationErrors.cvv = `CVV must be ${requiredcvvLength} digits`;
        } else {
  ...
  const handleSubmit = async event => {
    ...
    // cvv validation
    const requiredcvvLength = submissionCardType === 'American Express' ? 4 : 3;
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <Box sx={{ mb: 2 }}>
        <Cards number={formData.cardNumber} name={formData.cardName} expiry={formData.expiry} cvv={formData.cvv} focused={cardFocused} />
      </Box>
